...
return(<Link href={`/courses/${item.courseSlug}`}><div key={index} onClick={props.handleClose} className={styles.courseCard}><img src={item.imageLink}/>{item.courseName}</div></Link>)}
            }) : ''}
        </div></>:''}
