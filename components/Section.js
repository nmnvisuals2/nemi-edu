...
function Section(props){
    ...
    return(
        <section  ref={ref} id={props.id} data-parent={props?.isParent||false}  className={styles.section + " " + (props.visible ? styles.visible :'') + " " + (props.noMargin ? styles.noMargin : '') + " " + (props.noSpacing ? styles.noSpacing : '') + " " + (inView || props.noAnimation ? styles.inview : '' )  + " " + (props.fullWidth ? styles.fullWidth :'') + " " + (props.noMin ? styles.noMin :'') + (props.diff ? styles.diff : '')} style={{background:props?.background ? props?.background:''}}>
